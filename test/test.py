...
from qspi_pmod import qspi_pmod
from gamepad_pmod import send_gamepad_input, send_direct_input


@cocotb.test()
async def test_project(dut):

    # Enable verbose mode to debug a failing test
    VERBOSE = False

    # Flash data for QSPI Pmod
    QSPI_FLASH_BIN = "../data/asicle.bin"

    # Gamepad buttons to send
    GAMEPAD_INPUT = (
        # CRATE
        ["down"] *  3 + ["right"] +
        ["up"]   *  9 + ["right"] +
    ...
    async def check_line(expected_vsync):
        for i in range(H_TOTAL):
            hsync = dut.uo_out.value[7]
            vsync = dut.uo_out.value[3]
    ...
    async def capture_frame(frame_num, check_sync=True):
        dut._log.info(f"Rendering frame {frame_num}")
